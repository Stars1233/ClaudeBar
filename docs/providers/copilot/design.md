...
Facts about the two probes that the plan above takes for granted:

- **Billing mode**: `GET https://api.github.com/users/{username}/settings/billing/premium_request/usage` with `Accept: application/vnd.github+json` and `X-GitHub-Api-Version`. It keeps `usageItems` whose `product` contains "copilot" (case-insensitive) and totals them. The response doesn't include an allowance, so the user picks one (`copilot.monthlyLimit`, default 50). A 403 means the fine-grained PAT is missing **Plan: read**. A 404 means the user wasn't found or the token has no billing access.
- **Business/Enterprise can return empty `usageItems`** when the seat is provided by an org. The probe sets a "returned empty" flag, which the Settings card turns into a hint to try Copilot API mode, instead of turning on manual override automatically. "Zero usage" and "the API can't report usage" look the same in the response, so the probe doesn't treat one as the other.
- **Manual override** accepts a count or a percentage and is cleared when `timePeriod.{month,year}` in the response changes, i.e. when a new billing month starts.
